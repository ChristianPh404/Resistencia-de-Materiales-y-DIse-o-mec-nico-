...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textit{Pandeo:} Inestabilidad elástica por compresión en elementos esbeltos.
    ...
    \end{itemize}
    \item \textbf{Condiciones ambientales y del medio:} Temperatura de servicio (criogénica o elevada), exposición a atmósferas húmedas, salinas o fluidos corrosivos bajo tensión.
\end{enumerate}

Las propiedades mecánicas se dividen en tres grandes categorías:
\begin{itemize}
...
La \textbf{tensión o esfuerzo nominal} producido por la aplicación de una carga uniaxial representa la resistencia interna por unidad de superficie que ofrece una sección del material frente a las fuerzas exteriores aplicadas.
...
Al aplicar una carga uniaxial $P$, el elemento experimenta una variación dimensional en la dirección longitudinal de la fuerza denominada alargamiento o deformación total $\delta = l - l_0$. 

En ingeniería se define la \textbf{deformación unitaria nominal} ($\varepsilon$) como el cociente entre el incremento de longitud y la longitud inicial de referencia:
\begin{equation}
...
\begin{itemize}
    \item \textbf{Deformación elástica:} Si al retirar la carga el material recupera instantáneamente sus dimensiones iniciales $l_0$. A escala atómica, los enlaces químicos interatómicos se alargan o comprimen ligeramente sin que los átomos abandonen sus posiciones de equilibrio en la red cristalina.
    \item \textbf{Deformación plástica:} Si al retirar la carga el material experimenta una deformación permanente y residual. Se produce un desplazamiento irreversible de átomos mediante el movimiento colectivo de dislocaciones o deslizamiento de planos cristalográficos, alterando de forma permanente la microestructura del metal.
\end{itemize}
...
Consiste en someter una probeta normalizada a un estiramiento axial continuo y cuasiestático mediante un equipo de tracción servo-hidráulico o electromecánico dotado de célula de carga y extensómetro de contacto o láser, registrando en tiempo real la fuerza $P$ desarrollada y el alargamiento $\delta$ en la longitud calibrada.
...
\begin{itemize}
    \item \textbf{Probetas de sección circular:} Utilizadas comúnmente en piezas forjadas, barras y fundiciones. Presentan una longitud calibrada central reducida de diámetro $d_0 = 0.500'' \pm 0.010''$ ($12.7\text{ mm}$) y una longitud de calibración $L_0 = 2.000'' \pm 0.005''$ ($50.8\text{ mm}$). En general, se exige la condición geométrica:
    \begin{equation}
...
A partir de los registros continuos de carga $P$ y alargamiento $\delta$, se calculan la tensión nominal $\sigma = P/A_0$ y la deformación unitaria nominal $\varepsilon = \delta/L_0$. El perfil resultante define el comportamiento intrínseco del material:
\begin{itemize}
    \item \textbf{Materiales dúctiles} (ej. aceros de bajo carbono, cobre, aluminio recocido): Presentan un alargamiento elástico inicial lineal, una clara zona de deformación plástica uniforme generalizada, estricción localizada y un elevado alargamiento total antes de la fractura.
    \item \textbf{Materiales frágiles} (ej. fundiciones grises, cerámicos, aceros de altísima dureza): Prácticamente no exhiben deformación plástica; la rotura se produce de manera repentina poco después de superado el régimen elástico lineal, con deformación a rotura insignificante.
    \item \textbf{Materiales elastoméricos / polímeros termoplásticos:} Muestran curvas no lineales con grandes deformaciones, orientación molecular y estiramiento en frío.
\end{itemize}

\subsubsection{Módulo de Young ($E$) y Límite de Proporcionalidad ($\sigma_{LP}$)}
En el tramo inicial de la curva, el esfuerzo y la deformación crecen de forma estrictamente proporcional cumpliéndose la \textbf{Ley de Hooke}:
\begin{equation}
...
El \textbf{límite de proporcionalidad} ($\sigma_{LP}$) representa el nivel máximo de tensión a partir del cual la relación lineal deja de cumplirse:
\begin{equation}
...
Es la tensión máxima admisible sin que se produzcan deformaciones permanentes al cesar la carga. Si la carga supera $\sigma_{LE}$, al descargar el material queda una deformación plástica irreversible. En la práctica coincide de forma muy aproximada con el límite de proporcionalidad:
\begin{equation}
...
En los aceros suaves de bajo contenido en carbono se aprecian dos puntos de fluencia característicos:
\begin{itemize}
    \item \textbf{Punto de fluencia superior ($\sigma_{LFs} = P_A/A_0$):} Depende marcadamente de la velocidad de aplicación del ensayo y de la geometría de la probeta.
    \item \textbf{Punto de fluencia inferior ($\sigma_{LFi} = P_B/A_0$):} Es una propiedad intrínseca y constante del material, gobernada por el desanclaje de dislocaciones de las atmósferas de impurezas intersticiales (atmósferas de Cottrell).
\end{itemize}
...
A medida que la probeta sigue deformándose plásticamente, experimenta endurecimiento por deformación hasta alcanzar la carga máxima admisible $P_M$. La resistencia última a la tracción referida a la sección inicial es:
\begin{equation}
...
Corresponde al punto final donde la probeta fractura definitivamente bajo una carga $P_D$:
\begin{equation}
    \sigma_D = \frac{P_D}{A_0}
\end{equation}

\subsubsection{Ductilidad: Alargamiento y Estricción a la Rotura}
La ductilidad cuantifica el grado de deformación plástica que puede tolerar un material antes de la fractura. Se evalúa experimentalmente mediante dos parámetros tras unir cuidadosamente los extremos rotos de la probeta:
\begin{enumerate}
    ...
    \end{equation}
\end{enumerate}
donde $l_0$ y $A_0$ son la longitud calibrada y la sección inicial, y $l_f$ y $A_f$ son la longitud final entre puntos calibrados y la sección transversal mínima en el cuello de rotura. A mayores valores de \%AR y \%ES, mayor es la ductilidad del material y su seguridad frente a roturas catastróficas.
